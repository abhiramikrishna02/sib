...
export default function HeroSection() {
  ...
  const applySlots = useCallback(() => {
    cardRefs.current.forEach((card, i) => {
      ...
      if (raw >= N - 1) {
        const t = (raw - (N-1)) / 1
        props = { ...SLOTS[N-1], opacity: SLOTS[N-1].opacity*(1-t) }; z = 1
      } else if (raw < 0.35) {
        props = { ...SLOTS[0], opacity: SLOTS[0].opacity*(raw/0.35) }; z = 1
      } else {
        props = getSlotProps(raw); z = Math.max(1, Math.round(10 - Math.abs(raw-4)*1.2))
      }
      card.style.zIndex    = z
  ...
  const startLoop = useCallback(() => {
    ...
    const tick = ts => {
      ...
      offsetRef.current = (offsetRef.current + LOOP_SPEED * (ts - lastTRef.current) / 1000) % N
      lastTRef.current  = ts
      applySlots()
  ...
  const handleToggle = useCallback(() => {
    ...
    if (!expandRef.current) {
      ...
      cardRefs.current.forEach((card, i) => {
        ...
        const cx=a.x+(b.x-a.x)*t2, cy=a.y+(b.y-a.y)*t2
        const cr=a.rotate+(b.rotate-a.rotate)*t2, cs=a.scale+(b.scale-a.scale)*t2
  ...
  return (
    <section ref={sectionRef}
      ...
      {/* Gallery */}
      {showGallery && (
        <InfiniteGallery images={galleryImages} speed={1.2} visibleCount={12} scrollSourceRef={sectionRef}
          fadeSettings={{ fadeIn:{start:0.05,end:0.25}, fadeOut:{start:0.42,end:0.48} }}
          blurSettings={{ blurIn:{start:0,end:0.12}, blurOut:{start:0.42,end:0.48}, maxBlur:8 }}
          className="absolute inset-0 h-full w-full opacity-80 mix-blend-screen" style={{ zIndex:20 }} />
      )}
