        ...
        {slides.map((item, index) => (
          <div key={item.title} className="bengaluru-slide relative flex h-[100svh] w-[calc(100vw-2rem)] flex-shrink-0 items-center justify-center sm:w-[calc(100vw-3rem)] md:w-[74vw]">
            <div className="relative z-10 flex h-full w-full items-center px-5 py-6 sm:px-8 md:px-16 md:py-8 lg:px-20">
              <span className="bg-number pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none text-[34vw] font-black leading-none text-white/[0.03] md:text-[22vw]">0{index + 1}</span>
              <div className="relative z-10 grid w-full max-w-7xl grid-cols-1 items-center gap-5 md:grid-cols-[minmax(0,0.82fr)_minmax(320px,0.86fr)] md:gap-10 lg:gap-14">
                <div className="order-2 min-w-0 space-y-3 md:order-1 md:space-y-6">
                  <div className="space-y-3 md:space-y-4">
                    ...
                    <h3 className="max-w-[12ch] text-[clamp(2.15rem,11vw,3.7rem)] font-black uppercase italic leading-[0.92] text-white drop-shadow-[0_18px_45px_rgba(168,85,247,0.18)] md:text-[clamp(2.35rem,4vw,4.35rem)]">
                      <span className="bg-gradient-to-r from-white via-white to-violet-200 bg-clip-text text-transparent">{item.title}</span>
                    </h3>
                  </div>
                  <p className="max-w-[34rem] border-l-2 border-violet-400/20 pl-5 text-sm leading-relaxed text-white/56 md:pl-6 md:text-base lg:text-lg">{item.desc}</p>
    ...
    <CanvasErrorBoundary>
      <Canvas dpr={[1, 1.25]} gl={{ antialias: false, powerPreference: 'high-performance' }}>
        ...
        <Suspense fallback={null}>
          ...
          <ContactShadows position={shadowPosition} opacity={0.24} scale={compact ? 3 : 4.4} blur={1.6} color="#14091d" />
        </Suspense>
      ...
      <div className="container mx-auto grid grid-cols-1 items-center gap-4 md:gap-8 lg:grid-cols-2 lg:gap-10">
        <div className="relative max-w-2xl text-center pointer-events-auto lg:text-left">
          <div className="reveal relative mx-auto mb-1 h-[170px] w-full max-w-[300px] md:hidden">
            <div className="absolute inset-x-10 bottom-0 h-12 rounded-full bg-violet-500/18 blur-3xl" />
          ...
          <div className="reveal mb-5 inline-block rounded-full border border-violet-500/20 bg-violet-500/10 px-4 py-2 text-[9px] font-black uppercase tracking-[0.3em] text-violet-300 shadow-[0_0_20px_rgba(34,211,238,0.1)] backdrop-blur-xl sm:tracking-[0.4em] sm:text-[10px] md:mb-6">Our Identity</div>
